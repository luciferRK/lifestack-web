...
describe('spendingService', () => {
  ...
  it('calls categories endpoints', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/spending/categories', {
      params: { limit: 20, offset: 10 },
    });
    expect(api.post).toHaveBeenCalledWith('/spending/categories', {
      name: 'Rent',
      icon: '🏠',
      color: '#ff0000',
    });
  ...
  it('calls transactions endpoints', async () => {
    ...
    await spendingService.getTransactions(10, 0, {
      categoryId: 'cat1',
      accountId: 'acct1',
      fromDate: '2026-05-01',
      toDate: '2026-05-31',
    ...
    await spendingService.getTransactionSummary({
      fromDate: '2026-05-01',
      toDate: '2026-05-31',
      categoryId: 'cat1',
      accountId: 'acct1',
  ...
  it('calls budgets and trends endpoints', async () => {
    ...
    await spendingService.createBudget({
      amount: 500,
      category_id: 'cat1',
    ...
    expect(api.get).toHaveBeenNthCalledWith(1, '/spending/budgets', {
      params: { limit: 50, offset: 0, month_start: '2026-05-01' },
    });
    expect(api.post).toHaveBeenCalledWith('/spending/budgets', {
      amount: 500,
      category_id: 'cat1',
      month_start: '2026-05-01',
    });
    expect(api.patch).toHaveBeenCalledWith('/spending/budgets/b1', { amount: 600 });
    expect(api.get).toHaveBeenNthCalledWith(2, '/spending/analytics/trends', {
      params: { from: '2026-01-01', to: '2026-05-01' },
    });
  });
  ...
  it('calls recurring endpoints', async () => {
    ...
    await spendingService.createRecurring({
      amount: 15,
      category_id: 'cat1',
      frequency: 'monthly',
      interval: 1,
    ...
    expect(api.get).toHaveBeenNthCalledWith(1, '/spending/recurring', {
      params: { limit: 10, offset: 5, is_active: true },
    });
    expect(api.post).toHaveBeenCalledWith('/spending/recurring', {
      amount: 15,
      category_id: 'cat1',
      frequency: 'monthly',
      interval: 1,
    ...
    expect(api.get).toHaveBeenNthCalledWith(2, '/spending/recurring/upcoming', {
      params: { days: 45 },
    });
  });
});
