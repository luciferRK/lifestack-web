        ...
        multiMonthBudgets.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {multiMonthBudgets.map((b) => {
              ...
              return (
                <div
                  key={`${b.isGroup ? 'g' : 'c'}-${b.id}`}
                  className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-800/30 p-5 transition-all hover:border-slate-600 animate-in fade-in duration-200"
                >
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusBg} ${statusColor}`}
                    >
                  ...
                  <div className="mb-1 flex items-end justify-between">
                    <div>
                      ...
                      <p className="text-lg font-bold text-white">
                        {formatCurrency(b.spent, displayCurrency, currencyDisplayPreference)}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-slate-400">Total Budget</p>
                      <p className="font-semibold text-slate-300">
                        {formatCurrency(b.amount, displayCurrency, currencyDisplayPreference)}
                      </p>
                    </div>
                  </div>
                  ...
                  {b.monthly.length > 0 ? (
                    <div className="mt-4 border-t border-slate-700/50 pt-3">
                      ...
                      <div
                        ...
                        data-testid={`budget-monthly-${b.id || 'unassigned'}`}
                      >
                        {b.monthly.map((m) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${m.label}: ${Math.round(m.utilization)}% of ${formatCurrency(
                                m.amount,
                                displayCurrency,
                                currencyDisplayPreference,
                              )}`}
          ...
          {budgets?.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {budgets?.map((b) => {
                ...
                const theme = isGroup
                  ? getGroupTheme(b.category_group_id)
                  : getCategoryTheme(b.category_id);
                const spent = isGroup
                  ...
                  >
                    ...
                    <div className="mb-1 flex items-end justify-between">
                      <div>
                        ...
                        <p className="text-lg font-bold text-white">
                          {formatCurrency(spent, displayCurrency, currencyDisplayPreference)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-slate-400">Budget / month</p>
                        <p className="font-semibold text-slate-300">
                          {formatCurrency(bAmount, displayCurrency, currencyDisplayPreference)}
                        </p>
                      </div>
                    </div>
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                      {formatBudgetRangeLabel(b.start_month, b.end_month)}
                    </p>
                  </div>
                );
              })}
