    ...
    <PageShell>
      <PageHero
        ...
        actions={
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${isFetching || isBriefingFetching ? 'animate-spin' : ''}`}
            />
            Refresh
          </button>
        }
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl border border-rose-500/30 bg-rose-500/10 p-6 text-rose-100">
          ...
          <p className="mt-2 text-sm text-rose-100/80">
            Try refreshing the page or check the backend service.
          </p>
        </div>
      ) : data ? (
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            ...
            <MetricCard
              ...
              note={
                budgetSpotlight.length > 0
                  ? `${budgetSpotlight.length} group budget${
                      budgetSpotlight.length > 1 ? 's' : ''
                    } tracked below`
                  : 'No group budgets set'
              }
            ...
            <MetricCard
              ...
              value={
                data.investing.portfolio_value != null
                  ? formatCurrency(
                      data.investing.portfolio_value,
                      displayCurrency,
                      currencyDisplayPreference,
                      displayLocale,
                      decimalPlaces,
              ...
              note={
                data.investing.invested_value != null
                  ? `Invested ${formatCurrency(
                      data.investing.invested_value,
                      displayCurrency,
                      currencyDisplayPreference,
                      displayLocale,
                      decimalPlaces,
                    )} · Gain ${portfolioGainLabel}`
          ...
          {budgetSpotlight.length > 0 ? (
            <div className="mt-6">
              <section
                data-testid="dashboard-budget-spotlight"
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-rose-300/90">
                        You have {overdueCount} task{overdueCount > 1 ? 's' : ''} overdue. View your
                        todo list to update or complete them.
                      </p>
                    </div>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-amber-300/90">
                        {guardrailAlerts} active budget alert{guardrailAlerts > 1 ? 's' : ''}{' '}
                        require attention. Spending in some categories exceeds guardrail thresholds.
                      </p>
                    </div>
                ...
                {overspentCategories.length > 0 && (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-rose-300/90">
                        You have exceeded monthly budget limits in:{' '}
                        {overspentCategories.map((c) => c.name || 'Unknown').join(', ')}.
                      </p>
                    </div>
                    <ArrowRight className="h-4 w-4 shrink-0 mt-0.5 text-rose-400" />
                  </Link>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-blue-300/90">
                        Portfolio valuation status is '{data.investing.valuation_status}'. Some
                        asset holdings or exchange rates may not reflect real-time prices.
                      </p>
                    </div>
                        ...
                        <div className="flex-1">
                          <p
                            className={`font-semibold ${
                              isWarning ? 'text-amber-100' : 'text-cyan-100'
                            }`}
                          >
                            {insight.title}
                          </p>
                          {insight.body ? (
                            <p
                              className={`mt-1 text-sm ${
                                isWarning ? 'text-amber-300/90' : 'text-cyan-300/90'
                              }`}
                            >
                              {insight.body}
                            </p>
  ...
  >
    <div className="flex items-start justify-between gap-4">
      <div>
        ...
        <p data-testid={testId} className="mt-3 text-3xl font-bold tracking-tight text-white">
          {value}
        </p>
        <p className="mt-2 text-sm text-slate-300">{note}</p>
      </div>
      <div className="rounded-2xl border border-white/10 bg-white/10 p-3 text-white transition group-hover:bg-white/20">
        {icon}
      </div>
    </div>
  </Link>
